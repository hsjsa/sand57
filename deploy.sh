#bin/bash
appname=superior-workers-$RANDOM
if ! command -v heroku
then
    echo "Heroku could not be found"
    exit
fi
echo "Logged In On Heroku CLi? Type Y OR N."
read login
if ! ( [ "$login" == "Y" ] || [ "$login" == "y" ] )
then 
    echo "Login First"
...
then
    git add .
    git add -f token.pickle config.env fsub.py
    git commit -m "initial"
    git push heroku
    else
    echo "Pushing Accounts Folder Too"
    git add .
    git add -f token.pickle config.env fsub.py accounts accounts/*
    git commit -m "initial"
    git push heroku
fi
echo "Done"
